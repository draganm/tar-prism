...
| File | Responsibility |
|---|---|
| `tarprism.go` | package doc, prysm layout constants, `Index`/`Entry`, `ReadIndex`, `writeIndex`, `blobName` |
...
nix develop --command sh -c 'cd "$(mktemp -d)" && echo hi > a.txt && tar -cf in.tar a.txt && "$TMPDIR/tar-prism" decompose in.tar prysm && ls prysm prysm/blobs && cat prysm/recipe.json && "$TMPDIR/tar-prism" compose prysm out.tar && cmp in.tar out.tar && echo IDENTICAL'
