...
**Spec:** `docs/superpowers/specs/2026-09-03-tar-prism-design.md`
...
- Module path is `github.com/draganm/tar-prism`; the root package is named `tarprism`. Import it as `tarprism "github.com/draganm/tar-prism"`.
...
- Never leave a built binary in the tree. Use `go vet ./...` and `go build ./...` (multi-package form writes nothing). Never run `go build ./cmd/tar-prism` without `-o` into `$TMPDIR`, and delete it afterwards.
...
| File | Responsibility |
|---|---|
| `tarprism.go` | package doc, prism layout constants, `Index`/`Entry`, `ReadIndex`, `writeIndex`, `blobName` |
...
| `cmd/tar-prism/main.go` | urfave/cli/v2 app: `decompose`, `compose`, stdin/stdout via `-` |
...
| `compose_test.go` | compose error cases on tampered prisms |
| `system_test.go` | round trips of archives made by GNU tar and bsdtar, skipped if absent |
| `cmd/tar-prism/main_test.go` | CLI round trips through files and stdio, usage errors, failure cleanup |
...
Run: `nix develop --command go test ./...`
Expected: `ok  	github.com/draganm/tar-prism`
...
Add prism index types and ReadIndex
...
git commit -m "$(cat <<'EOF'
Test compose failure modes on tampered prisms
...
**Files:**
- Create: `cmd/tar-prism/main.go`
- Test: `cmd/tar-prism/main_test.go`
- Modify: `go.mod`, `go.sum` (add `github.com/urfave/cli/v2`)

**Interfaces:**
- Consumes: `tarprism.Decompose`, `tarprism.Compose`.
- Produces: `func newApp(stdin io.Reader, stdout io.Writer) *cli.App`; commands `decompose <input.tar|-> <prism-dir>` and `compose <prism-dir> <output.tar|->`.
...
Create `cmd/tar-prism/main_test.go`:
...
```bash
nix develop --command go build -o "$TMPDIR/tar-prism" ./cmd/tar-prism
nix develop --command sh -c 'cd "$(mktemp -d)" && echo hi > a.txt && tar -cf in.tar a.txt && "$TMPDIR/tar-prism" decompose in.tar prism && ls prism prism/blobs && cat prism/recipe.json && "$TMPDIR/tar-prism" compose prism out.tar && cmp in.tar out.tar && echo IDENTICAL'
rm -f "$TMPDIR/tar-prism"
```
...
git commit -m "$(cat <<'EOF'
Add tar-prism CLI with decompose and compose commands
